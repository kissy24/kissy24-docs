# Books

I have compiled a list of resources that I have personally authored and sites that I feel you should read.

## Setup

- [wsl セットアップ覚書](./setup/wsl_setup_worklist.md)
- [新しいPCをセットアップする](./setup/new_pc_setup.md)

## Python

- [Python 勉強メモ](./python/Python_勉強メモ.md)
- [Python の言語仕様について](./python/pythonの言語仕様について.md)
- [【Pytest】faulthandlerを無効化する](./python/【Pytest】faulthandlerを無効化する.md)
- [【Python】passとEllipsisの使い分けについて](./python/【Python】passとEllipsisの使い分けについて.md)
- [【Python】pytestで知っておきたいこと](./python/【Python】pytestで知っておきたいこと.md)
- [【入門】Pythonの型について理解する](./python/【入門】Pythonの型について理解する.md)

## GitHub

- [Issue のテンプレートについて](./books/github_issue_design.md)
- [GitHub Actions から Jira を操作する](./books/GitHubActionsを利用してJiraを操作する.md)

## Windows OS

- [Windows10が起動しなくなった時の対処法](./books/Windows10が起動しなくなった時の対処法.md)

## 勉強会

- [Docker&GitHub ハンズオン](./books/Docker＆Githubハンズオン.md)
- [Apache Spark 入門](./books/ApacheSpark入門.md)
- [matz さんの講演](./books/matz_ruby.md)
- [Developers Summit 2021 メモ](./books/DevelopersSummit2021_memo.md)

## チーム開発

### 外部参照

- [心理的安全性の構造 デブサミ2019夏 structure of psychological safety](https://www.slideshare.net/TokorotenNakayama/2019-structure-of-psychological-safety)
- [コードレビューの基準](https://shuuji3.xyz/eng-practices/review/reviewer/standard.html)

## テスト

### 外部参照

- [モックは必要悪で、しないにこしたことはない](https://blog.8-p.info/ja/2021/10/12/mock/)

## 関数型

- [関数型プログラミングが『銀の弾丸』であるという非常識な常識 2022](https://kentutorialbook.github.io/functionalprogramming2022/)
